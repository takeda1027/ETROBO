...
//   g_grayScaleBlueless = (cur_rgb.r * 77 + cur_rgb.g * 100 + (cur_rgb.b - cur_rgb.g) * 29) / 256; // B - G cuts off blue
 ...
 //       syslog(LOG_NOTICE, "gs = %d, MA = %d, x = %d", g_grayScale, ma_gs, x); //sano
    ...
    //スラロームが近い青でＢ２フラグ立てる
    if(cur_rgb.b - cur_rgb.r > 60 && !b2 && dis < 50 &&  cur_rgb.b<=255 &&cur_rgb.r<=255){
        b2 =true;
    ...
    printf(",r=%03u, g=%03u, b=%03u, b1=%d,b2=%d,slalom_flg=%d,time=%d\n",g_rgb.r, g_rgb.g, g_rgb.b,b1,b2,slalom_flg,clock->now());

    //printf(",b-r=%d,r=%03u, g=%03u, b=%03u, b1=%d,b2=%d,time=%s\n", g_rgb.b-g_rgb.r,g_rgb.r, g_rgb.g, g_rgb.b,b1,b2,clock->now());
