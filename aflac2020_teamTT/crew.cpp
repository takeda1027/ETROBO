...
//    if( cur_rgb.b - cur_rgb.r > 60 && right_angle && !b3){
    ...
    if(g_b3_turn_flg == 30){
        ...
        //黒を見つけたら、下向きのライントレース
        if(cur_rgb.r + cur_rgb.g + cur_rgb.b <= 100 && cur_rgb.r <=35 && cur_rgb.g <=35 && cur_rgb.b <=40){
            captain->decide(EVT_turnRight); // ここで直角ターン
            g_b3_turn_flg=40;
            captain->decide(EVT_go_30); // 直進
            roots_flg = 1;
        }
        
        //赤を見つけたら、赤を下へ、ライントレース
        if(cur_rgb.r - cur_rgb.b >=40 && cur_rgb.g <60){
            captain->decide(EVT_turnRight); // ここで直角ターン
            captain->decide(EVT_go_b3); // 物体に接近
            clock->sleep(300);
            prev_dis = getDistance();
        ...
        if(ran_flg == 2 && cur_rgb.r + cur_rgb.g - cur_rgb.b <= 130 && getDistance()-prev_dis>50){
            printf("赤色超えました2\n");
        ...
        if(ran_flg == 3 && cur_rgb.r + cur_rgb.g - cur_rgb.b <= 130 && getDistance()-prev_dis>50){
            printf("黄色超えました2\n");
    ...
    if(g_b3_turn_flg==60){   
        ...
        if(sonarSensor->getDistance() ==255 && ran_flg ==1){
            ran_flg=2;
        }
        if(sonarSensor->getDistance() < 160 && ran_flg ==2){
    ...
    if(g_b3_turn_flg==70 && getDistance()-prev_dis > 800){
