...
//    if( cur_rgb.b - cur_rgb.r > 60 && right_angle && !b3){
    ...
    if(g_b3_turn_flg ==10){
        //目標距離確認
        if(sonarSensor->getDistance() > 170 && sonarSensor->getDistance() <250){
            prev_deg = getAzimuth();
            yobi_deg = prev_deg;
            g_b3_turn_flg = 20;
        }
    ...
    //目標距離確認後の角度補正 sano_t
    if(g_b3_turn_flg == 20 && getAzimuth() - prev_deg > 13){
        g_b3_turn_flg = 30;
    ...
    if(g_b3_turn_flg == 30){
        //グリーン線１
        if(ran_flg == 0 && cur_rgb.r <= 13 && cur_rgb.b <= 50 && cur_rgb.g > 60){
            prev_dis = getDistance();
            ran_flg=5;
        }

        //グリーン線２
        if(ran_flg == 5 && cur_rgb.r <= 13 && cur_rgb.b <= 50 && cur_rgb.g > 60 &&  getDistance()-prev_dis > 50 ){
            int16_t x = getDistance();
            printf("緑、始点=%d,終点=%d,距離=%d\n",prev_dis,x,x-prev_dis);
            //累積角度をリセット
            azimuth = 0;

            //現在の角度を計算　中央に向かって０度　ラジアン
            deg_angle_r = atan2(sqrt((x-prev_dis)*(x-prev_dis)-155236),394);  //atan2(sqrt((x-prev_dis)^2-376^2),376); //376の底辺のべき乗は141376  
            //ラジアン→角度変換
            deg_angle=(360.0 * deg_angle_r / M_2PI);
            printf("角度deg_angle=%d,deg_angle_r=%lf\n",deg_angle,deg_angle_r);
            ran_flg = 6;
        }
        
        //黒を見つけたら、下向きの角度変更
        if(ran_flg==6 && cur_rgb.r + cur_rgb.g + cur_rgb.b <= 100 && cur_rgb.r <=35 && cur_rgb.g <=35 && cur_rgb.b <=40){
            captain->decide(EVT_turn_right_90); // 180度回頭
            printf("黒を見つけた\n");
            ran_flg = 7;
        }
        if(getAzimuth_f() - deg_angle > 90 && ran_flg == 7){
            captain->decide(EVT_go_b3); // 物体に接近
            printf("黒の回転終了\n");
            ran_flg =8;
            g_b3_turn_flg=40;
        ...
        //赤を見つけたら、赤を下へ、ライントレース
        if(cur_rgb.r - cur_rgb.b >=62 && cur_rgb.g <60){
            captain->decide(EVT_turn_180); // 180度回頭
            ran_flg = 2;
            printf("赤色超えました\n");
        }
        if(getAzimuth() - deg_angle > 90 && ran_flg == 2){
            captain->decide(EVT_go_b3); // 物体に接近
            printf("赤色超えました2\n");
            ran_flg =9;
            g_b3_turn_flg=40;
        }
        printf("角度差=%d,getAzimuth=%d,deg_angle=%d\n",getAzimuth()- deg_angle,getAzimuth(),deg_angle);
        ...
        if(ran_flg == 3 && cur_rgb.r + cur_rgb.g - cur_rgb.b <= 130 && getDistance() - prev_dis > 180 ){
            printf("黄色超えました2\n");
    ...
    if(g_b3_turn_flg==40){        
        //黄色を見つけたら直角ターン
        if(cur_rgb.r + cur_rgb.g - cur_rgb.b >= 160 && ran_flg==0){
            captain->decide(EVT_turnLeft); // ここで直角ターン
            prev_dis = getDistance();
            captain->decide(EVT_go_b3); // 物体に接近
            ran_flg = 1;
            printf("ここまで黄色エリア１\n");
        }
        if(ran_flg==1 && getDistance()-prev_dis > 180 && cur_rgb.r + cur_rgb.g - cur_rgb.b <= 130){
            printf("ここまで黄色エリア２\n");
            g_b3_turn_flg=50;
            ran_flg = 0;
            captain->decide(EVT_go_line_t); // 物体に接近
    ...
    if(g_b3_turn_flg==80){
        if(cur_rgb.r + cur_rgb.g <= 65 ){  //青または黒をみつけたら、ターン90度
    ...
    switch (state) {
        ...
        case ST_tracing_L:
            switch (event) {
                ...
                break;
                case EVT_turn_right_90:
                    lineTracer->turnC(1,30,0);
                break;
                case EVT_arm_up:
